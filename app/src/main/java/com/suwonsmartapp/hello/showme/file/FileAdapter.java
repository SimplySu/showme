    ...
    private boolean seeIfKnownType(File file, ImageView v) {
        ...
        for (int i = 0; i < 1; i++) {
            if (ext.equals(audio[i])) {
                Bitmap bm = FileThumbnail.getAudioThumbnail(mContext, file.getAbsolutePath());
                if (bm == null) {
                    return false;
                } else {
                    v.setImageBitmap(bm);
        ...
        for (int i = 0; i < 4; i++) {
            if (ext.equals(image[i])) {
                Bitmap bm = FileThumbnail.getImageThumbnail(mContext, file.getAbsolutePath());
                if (bm == null) {
                    return false;
                } else {
                    v.setImageBitmap(bm);
        ...
        for (int i = 0; i < 5; i++) {
            if (ext.equals(video[i])) {
                Bitmap bm = FileThumbnail.getVideoThumbnail(mContext, file.getAbsolutePath());
                if (bm == null) {
                    return false;
                } else {
                    v.setImageBitmap(bm);
